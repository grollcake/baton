...
12. `CLOSE` 승인을 받은 Director는 사용자 의견 중 장기 지침이 있으면 `.baton/GUIDANCE.md` 수정안을 제안합니다. `.baton/lesson-learned/` 추가안은 실제로 저지른 실수나 근본 원인·해법을 찾는 데 오래 걸린 것 중 다시 일어날 만한 것이 있을 때만 제안하며, 잠깐 들여다보면 알 수 있는 것은 교훈이 아닙니다. 대부분의 라운드에는 교훈이 없습니다. 사용자가 수락한 항목만 기록합니다.
...
`Solo` 작업은 사용자 완료 승인 없이 `RUN_DONE`으로 닫을 수 있습니다. 다만 장기 지침이나 위 기준을 넘는 교훈이 생겼다면 Director는 사용자에게 기록안을 제안하고, 사용자가 수락한 항목만 `GUIDANCE.md` 또는 `lesson-learned/`에 추가합니다.
