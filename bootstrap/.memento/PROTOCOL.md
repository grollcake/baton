...
artifacts if any. The `<memento-rules>...</memento-rules>` blocks in
`AGENTS.md` and `CLAUDE.md` must stay identical.

At the start of each recordable phase, its responsible role rereads:

1. `.memento/GUIDANCE.md`
2. the `.memento/LESSON-LEARNED.md` index
3. only records under `.memento/lesson-learned/` whose `Applies When` or
...
- `Direct`: minor localized edit, Memento AI bootstrap, or Memento AI update
...
Store artifacts in `.memento/runs/` using one stable
`<YYYYMMDD>-<HHMM>-<SLUG>` key:

- `.memento/runs/<YYYYMMDD>-<HHMM>-<SLUG>-PLAN.md`
- `.memento/runs/<YYYYMMDD>-<HHMM>-<SLUG>-RUN-<NN>.md`
...
match `memento.log`; all artifacts for a task use the PLAN key, and each REVIEW
...
- Never store secrets, credentials, customer data, personal information,
  sensitive internal information, or production secrets under `.memento/`.

## Git And Updates

Commit `.memento/` to Git. Do not add it to `.gitignore`. When updating,
read `.memento/HOW-TO-UPDATE.md` first. Bootstrap and update are recording
targets, not excluded meta work.

Memento AI uses the native `.memento/bin/memento[.exe]` Go binary and
